...
public class Node {
	...
	Node (Node node) // copy constructor no args
	{
		int i, j;
		state = new int[4][4];
		for(i=0;i<4;i++)
	...
	public void findChilds(Node node, LinkedList<Node> queue, LinkedList<Node> visited_nodes)
	{
		Node child1, child2, child3, child4;
		int i, j;
		if(node.blank_cell_x + 1 <= 3)
		{
			child1 =  new Node(node);
			...
			if(!child1.check_visited(visited_nodes))
				queue.addLast(child1);
		}
		if(node.blank_cell_y + 1 <= 3)
		{
			child2 =  new Node(node);
		...
		if(node.blank_cell_x - 1 >= 0)
		{
			...
			if(!child3.check_visited(visited_nodes))
				queue.addLast(child3);
		}
		if(node.blank_cell_y - 1 >= 0)
		{
			child4 =  new Node(node);
